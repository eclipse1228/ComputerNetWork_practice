...
int main(int argc, char *argv[])
{
        ...
        while (1) {
                ...
                // 문제는 달력이 총 얼마나 출력되어야하는지가 문제다. 즉 상대방이 얼마나 줄지 알아야한다.
                // 그런데. 28일,30일,31일도 있어서.. 어렵다. 그러면.. 마지막 28일이라면,. 그만큼 서버에서 29 30, 31일만큼 공백을 추가해서 넘겨야한다. (O) : 이거 괜찮은 방법이었다.
                  // 하지만 늘 통신에 늘 규칙적인 데이터의 크기가 이동하진 않는다. 즉 과제에서 해결하길 바라는 지점이 아니다.
        
                // 그러지 않으면.. 31일 크기만 받으면 되지 않나.? (X) : 만약 31일치 받으려고 요청했는데, 28일이면 서버에서 줄때까지 기다리기 때문에 무한 대기에 빠지게됨. 
